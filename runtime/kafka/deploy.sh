...
IP=$(echo "$ENDPOINTS_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['kafka']['ip'])")

echo "Deploy runtime kafka to $IP via bastion $BASTION_HOST"

REMOTE_DIR_BASE="${REMOTE_DIR_BASE:-/home/ec2-user/runtime}"
REMOTE_DIR="${REMOTE_DIR_BASE}/kafka"
